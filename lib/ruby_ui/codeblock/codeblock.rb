...
module RubyUI
  class Codeblock < Base
    ...
    def initialize(code, syntax:, clipboard: true, clipboard_success: 'Copied!', clipboard_error: 'Copy failed!',
                   **attrs)
      @code = code
      ...
      @code = @code.gsub(/(?:^|\G) {2}/m, "\t") if @syntax == :ruby || @syntax == :html

      super(**attrs)
    end

    def view_template
      # For faster load times, move this to the head of your document. (Also move ROUGE_CSS value to head of document)
      style do
        ROUGE_CSS
      end
      if @clipboard
      ...
      RubyUI.Clipboard(success: @clipboard_success, error: @clipboard_error, class: 'relative') do
        ...
        div(class: 'absolute top-2 right-2') do
          RubyUI.ClipboardTrigger do
            RubyUI.Button(variant: :ghost, size: :sm, icon: true,
                          class: 'text-white hover:text-white hover:bg-white/20') do
              clipboard_icon
            end
          end
        end
